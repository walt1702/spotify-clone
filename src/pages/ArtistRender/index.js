...
import { fetchArtistAlbums, fetchArtistDetails, fetchArtistTracks, fetchFollowedArtists, fetchRelatedArtists, followArtist, initUnfollowArtist, setFollowedArtists, unfollowArtist } from '../../state/ducks/userCollection/actions';
...
function ArtistRender()
{
    ...
    const handleClick = () =>{
        ...
        {
            setFollow(false);
            dispatch(unfollowArtist(token,artistId));
            dispatch(initUnfollowArtist(artistId));
        }
        else
        {
            setFollow(true);
            dispatch(followArtist(token,artistId));
			dispatch(setFollowedArtists([artist.details]));
        }
    }
    return (
        <div>
